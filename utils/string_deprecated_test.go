...
var rawStrArrDeprecated = []string{
...
var expectedStrArrDeprecated = []string{
...
func TestWordwrap(t *testing.T) {
	for i := 0; i < len(rawStrArrDeprecated); i++ {
		finalStr := Wordwrap(rawStrArrDeprecated[i], 64, "\n")
		t.Log(finalStr, "<---->", expectedStrArrDeprecated[i])
		assert.Equal(t, expectedStrArrDeprecated[i], finalStr)
	}
}
...
var a2zLowercaseDeprecated = "abcdefghijklmnopqrstuvwxyz"
...
func TestBytesStrExchange(t *testing.T) {
	t.Run("TestBytesStrExchange", func(t *testing.T) {
		for _, str := range expectedStrArrDeprecated {
