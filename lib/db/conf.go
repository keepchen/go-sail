...
type Conf struct {
	...
	AllowGlobalUpdate                        bool               `yaml:"allow_global_update" toml:"allow_global_update" json:"allow_global_update"`                                                                               //允许全局更新(不带where条件)
	SkipDefaultTransaction                   bool               `yaml:"skip_default_transaction" toml:"skip_default_transaction" json:"skip_default_transaction"`                                                                //跳过默认事务
...
type Logger struct {
	...
	SkipCallerLookup          bool   `yaml:"skip_caller_lookup" toml:"skip_caller_lookup" json:"skip_caller_lookup"`                                  //是否跳过调用者检查
	IgnoreRecordNotFoundError bool   `yaml:"ignore_record_not_found_error" toml:"ignore_record_not_found_error" json:"ignore_record_not_found_error"` //忽略未找到记录错误
	Colorful                  bool   `yaml:"colorful" toml:"colorful" json:"colorful"`                                                                //是否带颜色打印
}
...
type MysqlConf struct {
	Read  MysqlConfItem `yaml:"read" toml:"read" json:"read"`    //读实例
	Write MysqlConfItem `yaml:"write" toml:"write" json:"write"` //写实例
}
...
type MysqlConfItem struct {
	...
	ParseTime bool   `yaml:"parseTime" toml:"parseTime" json:"parseTime"` //是否解析时间
	Loc       string `yaml:"loc" toml:"loc" json:"loc" default:"Local"`   //位置
}

type PostgresConf struct {
	Read  PostgresConfItem `yaml:"read" toml:"read" json:"read"`    //读实例
	Write PostgresConfItem `yaml:"write" toml:"write" json:"write"` //写实例
}
...
type PostgresConfItem struct {
	...
	TimeZone string `yaml:"timezone" toml:"timezone" json:"timezone"` //时区
}

type SqlserverConf struct {
	Read  SqlserverConfItem `yaml:"read" toml:"read" json:"read"`    //读实例
	Write SqlserverConfItem `yaml:"write" toml:"write" json:"write"` //写实例
}
...
type SqliteConf struct {
	Read  SqliteConfItem `yaml:"read" toml:"read" json:"read"`    //读实例
	Write SqliteConfItem `yaml:"write" toml:"write" json:"write"` //写实例
}

// SqliteConfItem sqlite配置
type SqliteConfItem struct {
	File string `yaml:"file" toml:"file" json:"file"` //数据库文件
}

type ClickhouseConf struct {
	Read  ClickhouseConfItem `yaml:"read" toml:"read" json:"read"`    //读实例
	Write ClickhouseConfItem `yaml:"write" toml:"write" json:"write"` //写实例
}
